...
def setup_logger(name, log_file, level=logging.DEBUG):
    '''
    add logging for the server
    return: a logger
    input:
    - log_file: file for saving logging info.
    '''
    ...
    # uncomment the following if you want to saw all debug info in the console
...
def load_image_url_numpy(image_url):
    '''
    load url image to numpy array format
    '''
...
def load_image_url_string(image_url):
    '''
    load url image to string format
    '''
...
def time_wrapper(logger):
    '''
    decorater for timeing.
    input:
    - logger: specifiy a logger to save logging info
    '''
    def timeit(method):
        def timed(*args, **kw):
            ts = time.time()
...
# get warn-up
# It not necessary but I think it will be make sure every testing will not be delated as the first test always slow.
n = 1
